...
4. Product semantics such as Note versus chat, final output versus delta, and Workspace instance identity versus permissions must be decided independently.
...
| #10 Workspace identity and permissions | The resolved `instanceKey` chooses Workspace and Session reuse; it must not silently force read-only permissions. | Test default task isolation, explicitly named cross-task reuse, and permissions as independent axes. |
...
| #12 TaskAgentSession wording | Documentation tied to the current Task was mistaken for a forced task-isolated Provider Session. | Review names/comments against runtime behavior. Test `taskId`, resolved `instanceKey`, and Provider Session lifetime separately. |
