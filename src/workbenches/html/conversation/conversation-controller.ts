/**
 * Conversation state machine for the HTML assistant overlay (renderer).
 *
 * Owns the conversation identity, message stream, generation task event
 * subscription, persistence, and restore/delete/new flows. The React view
 * (`ConversationOverlay.tsx`) renders only UI and delegates interaction
 * here. Optional `assistant-delta` events enhance the in-progress display;
 * the completed task result is authoritative.
 */
...
export interface ConversationControllerState {
  ...
  readonly history: readonly HtmlConversationEntry[];
  readonly input: string;
  readonly pendingAnchor: JsonValue | undefined;
...
export interface ConversationControllerActions {
  ...
  readonly setPendingAnchor: (anchor: JsonValue | undefined) => void;
  readonly setErrorText: (error: string | undefined) => void;
  ...
  readonly handleCancelAnswer: () => void;
}
...
function entryToMessages(entry: HtmlConversationEntry): ConversationDisplayMessage[] {
...
export function useConversationController({
  ...
  const createArchiveId = options.createConversationId ?? createConversationId;
  ...
  const replaceMessages = (next: ConversationDisplayMessage[]) => {
  ...
  const updateMessages = (
    update: (current: ConversationDisplayMessage[]) => ConversationDisplayMessage[],
  ...
  const deleteEntry = (entry: HtmlConversationEntry) => {
