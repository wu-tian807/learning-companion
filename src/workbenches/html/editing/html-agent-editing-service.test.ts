...
import type { WorkbenchStateRecord } from '../../../main/workbench/workbench-state-database';
...
function task(taskId: string, completed = false) {
  return {
    id: taskId,
    projectId: 'project-1',
...
describe('HtmlAgentEditingService Workbench persistence', () => {
  it('restores the draft and groups a completed GenerationTask as one history step', async () => {
    ...
    const states = new Map<string, WorkbenchStateRecord>();
    const data = new Map<string, WorkbenchStateDataRecord>();
    const stateDatabase = {
      get: vi.fn(async (assetId: string) => states.get(assetId)),
      save: vi.fn(async (record: WorkbenchStateRecord) => {
        states.set(record.assetId, structuredClone(record));
      }),
      delete: vi.fn(async (assetId: string) => {
        states.delete(assetId);
      }),
    };
  ...
  it('groups every replace from one task into one undo and redo step', async () => {
    const original =
  ...
  it('settles pending edits from the existing GenerationTask subscription', async () => {
    const original = '<html><body><p id="target">Before</p></body></html>';
    const bytes = new TextEncoder().encode(original);
    let listener:
    ...
    listener?.({ type: 'task-completed', snapshot: task('task-1', true) });

    await vi.waitFor(async () => {
      await expect(service.review('project-1', 'asset-1')).resolves.toMatchObject(
        { entries: [{ taskId: 'task-1' }], pendingChanges: [] },
      );
    });
  });
  ...
  it('materializes the current draft for the next GenerationTask reference', async () => {
    ...
    const path = await service.materializeDraft('project-1', 'asset-1');
    try {
