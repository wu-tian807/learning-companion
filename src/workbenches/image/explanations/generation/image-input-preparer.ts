...
export interface PreparedImageExplanationInputs {
...
export async function prepareImageExplanationInputs(
  ...
  if (
    ...
    throw new AppError('DATA_INTEGRITY_ERROR', {
      cause: new Error('图片尺寸无效或像素总量超过安全限制'),
    });
  }
  if (
    sourceWidth !== target.anchorPayload.sourceWidth ||
    sourceHeight !== target.anchorPayload.sourceHeight
  ) {
    throw new AppError('CONTENT_CHANGED_EXTERNALLY', {
      cause: new Error('图片尺寸与创建兴趣区域时不一致'),
    });
  }

  const outputDirectory = join(workspacePath, 'image-explanation-inputs');
  ...
  markedContext.fillRect(0, y + height, overviewSize.width, overviewSize.height - y - height);
  markedContext.fillRect(0, y, x, height);
  markedContext.fillRect(x + width, y, overviewSize.width - x - width, height);
  markedContext.strokeStyle = '#ff3b30';
  markedContext.lineWidth = Math.max(3, Math.round(Math.min(overviewSize.width, overviewSize.height) / 300));
  ...
  const cropRight = Math.min(sourceWidth, Math.ceil(regionX + regionWidth + paddingX));
  const cropBottom = Math.min(sourceHeight, Math.ceil(regionY + regionHeight + paddingY));
