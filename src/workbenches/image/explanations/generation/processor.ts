...
回答必须使用中文，准确、清楚、适合普通读者。围绕用户当前问题直接回答，并在同一对话的后续追问中继承已有图片语境。优先直接说明“选中的是什么”，然后解释它在整图中的作用、与周围内容的关系及关键细节。不要机械复述要求，不要编造看不清的文字、身份、因果或背景；存在歧义时明确说明。
    ...
    >
{
  ...
  async process(
    ...
    if (target) {
      ...
      userMessage = Object.freeze({
        ...
        content: Object.freeze([
          ...context.preparedUserMessage.content,
    ...
    if (!target) throw new AppError('DATA_INTEGRITY_ERROR');
    context.reportStatus('回答已生成，正在保存图片解释标注…');
    ...
    return Object.freeze({ ...commonResult, attachmentId: attachment.id });
  }
}
