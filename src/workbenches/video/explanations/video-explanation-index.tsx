...
export function orderVideoExplanations(
  explanations: readonly VideoExplanationView[],
...
export function summarizeVideoExplanation(
  answer: string,
  maxCharacters = 40,
): string {
...
export function formatVideoExplanationTime(timeSeconds: number): string {
...
function explanationStatusLabel(explanation: VideoExplanationView): string {
...
export function VideoExplanationIndex({
  explanations,
  activeExplanationId,
  onActivate,
  onDelete,
  ...
  readonly onDelete: (explanation: VideoExplanationView) => void;
  readonly onClose: () => void;
}) {
  const orderedExplanations = orderVideoExplanations(explanations);
  return (
    <aside
      aria-label="视频标注索引"
        ...
        <div>
          ...
          <p className="mt-0.5 text-[10px] text-slate-600">
            {explanations.length > 0
              ? `${explanations.length} 条 · 点击跳转到对应画面`
              : '尚无标注'}
          </p>
        </div>
        <button
