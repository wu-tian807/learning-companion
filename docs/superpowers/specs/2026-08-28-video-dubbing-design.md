...
- 不同 speaker 的重叠 Cue 始终保持为独立 Phrase；写入声轨时只在模型识别出的真实重叠区做衰减叠加，不能由后写 Phrase 覆盖先写声音，也不额外扩大发生重叠的时间窗；
...
本次移除说话人模型时保留配音组件原版本目录并提升安装格式号。旧安装会先进入可清理
状态；清理后重装会在原位置得到不含 pyannote/CAMPPlus 的包，不遗留另一份旧模型目录。
