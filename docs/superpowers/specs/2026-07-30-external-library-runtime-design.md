...
- `.downloads/<library-id>/<package-fingerprint>/` 使用可信清单指纹形成稳定路径，
  不绑定某一次安装任务 UUID；
- 断网、进程退出或崩溃后保留未完成文件，重试时优先使用 HTTP Range 续传；
- 服务端不接受 Range 时只从头下载当前资源，已经校验完成的同包资源继续复用；
- 安装成功、用户显式取消或移除组件后删除对应下载缓存；超过 7 天没有活动的
  下载缓存自动回收；
- 解包和验证只在 `.staging` 进行；
- 超过 24 小时没有任何文件活动的孤儿 staging 在启动时自动回收；
- 只有完整验证后才原子移动到正式版本目录；
...
- 网络失败可以重试；
- 重试必须复用已校验资源；已有部分文件时发送 `Range: bytes=<size>-`，仅接受
  与本地偏移、期望总大小一致的 `206 Content-Range`；
- 服务端返回完整 `200` 或拒绝 Range 时可以重下当前资源，但不能清除同包其他
  已校验资源；
- 续传完成后仍必须重新校验完整文件大小和 SHA-256；
- 用户可以取消下载；
- 用户显式取消视为放弃本次下载并立即释放缓存；应用正常退出或异常中断则保留
  可恢复数据；
- 校验失败必须删除 staging 和不可信下载；
- 安装失败不能覆盖已有可用版本；
- 应用退出后残留的下载在保留期内可恢复，过期下载与孤儿 staging 在后续启动清理；
...
- 模拟断网、Range 续传、服务端忽略或拒绝 Range、磁盘不足、校验失败和应用重启；
- 验证 Bundle 后续资源失败时不重复下载已经完成的前序大资源；
- 验证成功、显式取消、移除和过期回收释放缓存，而应用退出保留可恢复 partial；
- 模拟目标目录已有合法安装和未知文件；
- 验证迁移失败不会改变 settings；
